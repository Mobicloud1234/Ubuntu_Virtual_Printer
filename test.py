...
def extract_invoice_fields(kvs):
    return {
        ...
        "Invoice Date": match_key(kvs, ["Dated", "Invoice Date"]),
        "GSTIN": match_key(kvs, ["GSTIN"]),
        "Buyer Name": match_key(kvs, ["Buyer", "Customer", "Consignee"]),
        "Buyer Contact": match_key(kvs, ["Mobile", "Contact"]),
        "Total Amount": match_key(kvs, ["Total", "GrandTotal","Invoice Total", "Amount Payable"]),
        "HSN Code": match_key(kvs, ["HSN", "HSN/SAC"]),
        "CGST": extract_percentage(match_key(kvs, ["CGST"])),
        "SGST": extract_percentage(match_key(kvs, ["SGST"])),
...
def main():
    ...
    try:
        ...
        # physical_printer = "LBP6030-6040-6018L-2"
